...
# RalphLoop Stop Hook
# Prevents session exit when a RalphLoop is active
...
# Session isolation: state file is project-scoped, but Stop hook
...
# Disable errexit temporarily so jq failure can be caught by $? check.
set +e
